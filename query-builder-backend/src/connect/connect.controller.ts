...
export class ConnectController {
  ...
  async connect(@Body() credentials: DatabaseCredentials) {
    try {
      const result =
        await this.connectionManager.connectToDatabase(credentials);
      return result;
    } catch (error) {
  ...
  async getSchemaMetadata(@Body() credentials: DatabaseCredentials) {
    ...
    const query: Query = {
      ...
      queryParams: {
        ...
        table: 'schemata',
        columns: ['schema_name'],
        condition: `schema_name NOT IN ('information_schema', 'mysql', 'sys', 'performance_schema')`,
        sortParams: {
          column: 'schema_name'
        }
      }
  ...
  async getFieldMetadata(@Body() fieldQuery: FieldQuery) {
    ...
    const query: Query = {
      ...
      queryParams: {
        ...
        table: 'columns',
        columns: ['column_name'],
        condition: `table_schema = ${fieldQuery.schema} AND table_name=${fieldQuery.table}`,
        sortParams: {
          column: 'column_name'
        }
      }
