    ...
    async connectToDatabase(db_id: string, session: Record<string, any>): Promise<ConnectionStatus> {
        {
            return new Promise(async (resolve, reject) => {
              ...
              if (session.host === host) {
                ...
                this.logger.log(`[Reconnecting] ${session.id} connected to ${host}`, PostgresConnectionManagerService.name);
                return resolve({
                  success: true,
                ...
                if (session.host !== undefined) {
                  ...
                  this.logger.log(`[Connection Disconnected] ${session.id}`, PostgresConnectionManagerService.name);
