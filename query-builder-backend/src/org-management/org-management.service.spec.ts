...
const { setTestData, setTestError } = require('../supabase/supabase.ts');
  ...
  describe('createHash', () => {
    ...
    it('should rethrow the error generated by the Supabase client when inserting the hash into the organisation', async () => {
      ...
      jest.spyOn(service, 'createHash_H3').mockResolvedValue({data: [{org_id: '0000', hash: '0000'}], error: new Po);
  ...
  describe('addMember', () => {
    ...
    it('should rethrow the error generated by the Supabase client when the user is not logged in or cannot be found', async () => {
      ...
      await service.addMember({ org_id: '0000', user_id: '0001' }).catch((error) => {
        expect(error).toBeDefined();
        expect(error).toHaveProperty('message', 'Not logged in');
        expect(error).toHaveProperty('status', 401);
      });
    ...
    it('should update the verified status of the user to true if the user is already a member of the organisation', async () => {
      ...
      const { data } = await service.addMember({ org_id: '0000', user_id: '0001' });
      expect(data).toBeDefined();
