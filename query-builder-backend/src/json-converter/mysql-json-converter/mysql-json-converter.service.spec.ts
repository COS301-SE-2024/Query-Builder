...
describe('MysqlJsonConverterService', () => {
  ...
  it('should be able to convert compound conditions with AND and OR', () => {
    ...
    expect(result).toEqual(
      " WHERE (`name` = 'value' AND `age` > 18 AND (`city` = 'New York' OR `status` != 'inactive'))"
    );
  });
  ...
  it('should be able to convert queries with multiple columns selected', () => {
    const queryParams: QueryParams = {
      ...
      table: {
        name: 'users',
        columns: [{ name: 'id' }, { name: 'first_name' }, { name: 'last_name' }]
      }
    };

    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name` FROM `sakila`.`users`';
  ...
  it('should be able to convert queries with aggregation and aliasing', () => {
    const queryParams: QueryParams = {
      ...
      table: {
        name: 'users',
        columns: [
          { name: 'id', aggregation: AggregateFunction.COUNT, alias: 'Number' }
        ]
      }
    };

    const expectedQuery =
      'SELECT COUNT(`users`.`id`) AS `Number` FROM `sakila`.`users`';
  ...
  it('should be able to convert queries with joins', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `actors`.`role` FROM `sakila`.`users` JOIN `sakila`.`actors` ON `users`.`id`=`actors`.`user_id`';
  ...
  it('should be able to convert queries with sorting in descending order', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name` FROM `sakila`.`users` ORDER BY `first_name` DESC';
  ...
  it('should be able to convert queries with sorting in ascending order', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name` FROM `sakila`.`users` ORDER BY `first_name` ASC';
  ...
  it('should be able to convert queries using pagination', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name` FROM `sakila`.`users` LIMIT 10 OFFSET 20';
  ...
  it('should be able to convert queries using pagination and a where condition', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name` FROM `sakila`.`users` WHERE `age` > 18 LIMIT 10 OFFSET 20';
    const result = service.convertJsonToQuery(queryParams);
  ...
  it('should be able to convert queries using pagination, where, group by, and having conditions', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name`, AVG(`users`.`age`) FROM `sakila`.`users` GROUP BY `users`.`id`, `users`.`first_name`, `users`.`last_name` HAVING AVG(`users`.`age`) > 18 LIMIT 10 OFFSET 20';
    const result = service.convertJsonToQuery(queryParams);
  ...
  it('should be able to convert queries using pagination, where, and group by conditions', () => {
    ...
    const expectedQuery =
      'SELECT `users`.`id`, `users`.`first_name`, `users`.`last_name`, `users`.`age` FROM `sakila`.`users` WHERE `age` > 18 LIMIT 10 OFFSET 20';
    const result = service.convertJsonToQuery(queryParams);
  ...
  it('Should be able to convert a query with a join and a having, with aggregate in first table', () => {
    ...
    expect(result).toEqual(
      'SELECT COUNT(`city`.`city_id`) AS `Number of cities per country`, `country`.`country` FROM `sakila`.`city` JOIN `sakila`.`country` ON `city`.`country_id`=`country`.`country_id` GROUP BY `country`.`country` HAVING COUNT(`city`.`city_id`) > 10'
    );
  });
  ...
  it('Should be able to convert a query with a join and a having, with aggregate in second table', () => {
    ...
    expect(result).toEqual(
      'SELECT `country`.`country`, COUNT(`city`.`city_id`) AS `Number of cities per country` FROM `sakila`.`country` JOIN `sakila`.`city` ON `country`.`country_id`=`city`.`country_id` GROUP BY `country`.`country` HAVING COUNT(`city`.`city_id`) > 10'
    );
  });
  ...
  it('Should not return GroupBY if there is no aggregates', () => {
    ...
    const result = service.convertJsonToQuery(jsonData);
    expect(result).toEqual(
      'SELECT `city`.`city_id`, `country`.`country` FROM `sakila`.`city` JOIN `sakila`.`country` ON `city`.`country_id`=`country`.`country_id` WHERE `city_id` > 10'
    );
  });
  ...
  it('Should convert a query finding country names starting with B', () => {
    ...
    expect(result).toEqual(
      "SELECT `country`.`country` FROM `sakila`.`country` WHERE `country` LIKE 'B%'"
    );
  });
});
