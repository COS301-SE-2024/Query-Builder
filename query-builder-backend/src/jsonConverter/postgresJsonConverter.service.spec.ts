...
import { postgresJsonConverter } from './postgresJsonConverter.service';
import { QueryParams, AggregateFunction, ComparisonOperator } from '../interfaces/intermediateJSON';

describe('postgresJsonConverter', () => {
  let service: postgresJsonConverter;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [postgresJsonConverter],
    }).compile();

    service = module.get<postgresJsonConverter>(postgresJsonConverter);
  });
  ...
  //------------------------------------- Individual functions tests -------------------------------------//
    ...
    it('should generate a postgres query for finding country names starting with B', () => {
        ...
        const expectedQuery = 'SELECT "country" FROM "country" WHERE "country" LIKE \'B%\'';
    ...
    it('should generate WHERE clause with numeric value', () => {
        ...
        const expectedQuery = 'SELECT "population" FROM "population" WHERE "population" > 10000';
        const result = service.convertJsonToQuery(jsonData);
    ...
    it('should handle single quotes in string values for WHERE clause', () => {
        ...
        const expectedQuery = 'SELECT "name" FROM "people" WHERE "name" LIKE \'O\'\'Reilly\'';
        const result = service.convertJsonToQuery(jsonData);
    ...
    it('should generate GROUP BY clause with columns', () => {
        ...
        const expectedQuery = 'SELECT "country" FROM "country" GROUP BY "country", "state"';
        const result = service.convertJsonToQuery(jsonData);
    ...
    describe('convertJsonToQuery', () => {
        it('should throw an error for invalid language', () => {
            const jsonData: QueryParams = {
                ...
                query_type: 'select',
                table: { name: 'test_table', columns: [{ name: 'col1' }, { name: 'col2' }] },
                databaseName: 'sakila'
            };

            expect(() => service.convertJsonToQuery(jsonData)).toThrowError('Invalid language');
        });
        ...
        it('should throw an error for unsupported query type', () => {
            ...
            expect(() => service.convertJsonToQuery(jsonData)).toThrowError('Unsupported query type');
        });
        ...
        it('should throw an error for missing table columns', () => {
            ...
            expect(() => service.convertJsonToQuery(jsonData)).toThrowError('Invalid query');
        });
        ...
        it('should return a correct SQL query for valid input', () => {
            const jsonData: QueryParams = {
                ...
                query_type: 'select',
                table: { name: 'test_table', columns: [{ name: 'col1' }, { name: 'col2' }] },
                condition: { column: 'col1', operator: '=', value: 'value1' },
            };
        
            const result = service.convertJsonToQuery(jsonData);
            const expectedQuery = 'SELECT "col1", "col2" FROM "test_table" WHERE "col1" = \'value1\'';
    ...
    describe('generateSelectClausePost', () => {
        it('should return "*" if table or columns are missing', () => {
            ...
            const resultMissingTable = service.generateSelectClausePost(jsonDataMissingTable);
            const resultMissingColumns = service.generateSelectClausePost(jsonDataMissingColumns);
